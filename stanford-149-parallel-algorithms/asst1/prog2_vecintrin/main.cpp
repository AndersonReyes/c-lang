...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  //
  //
  // * use a while loop to perform each vector exp mult one by one until the
  // mask is all zeros

  __cs149_mask maskAll;
  // will track which exponents still need to be multiplied
  __cs149_mask maskPositiveExponent;
  __cs149_mask maskLargeValues;
  __cs149_vec_int zero = _cs149_vset_int(0);
  __cs149_vec_int one = _cs149_vset_int(1);
  __cs149_vec_float y;
  __cs149_vec_int exponentCounter;
  __cs149_vec_float maxValues = _cs149_vset_float(9.999999);

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    // have to do this because we can't multiply float vector by int vector
    float exponentsAsFloat[N];
    for (int j = 0; j < N; ++j) {
      exponentsAsFloat[j] = exponents[j];
    }

    maskAll = _cs149_init_ones(N);
    maskLargeValues = _cs149_init_ones(0);

    __cs149_vec_float result;
    _cs149_vload_float(result, values, maskAll);
    _cs149_vload_float(y, exponentsAsFloat + i, maskAll);
    _cs149_vload_int(exponentCounter, exponents + i, maskAll);
    //_cs149_vsub_int(exponentCounter, exponentCounter, one, positiveExponent);

    // for (int k = 0; k < 4; ++k) {
    //   addUserLog(std::format("result[{}] = {}", k, result.value[k]).c_str());
    // }

    // for (int k = 0; k < 4; ++k) {
    //   addUserLog(std::format("y[{}] = {}", k, y.value[k]).c_str());
    // }

    maskPositiveExponent = _cs149_init_ones(N);

    // while we have some exponent counters > 0
    while (_cs149_cntbits(maskPositiveExponent) > 0) {
      // check which exponent counters are > 0
      _cs149_vgt_int(maskPositiveExponent, exponentCounter, zero, maskAll);

      // multiply exponent
      _cs149_vmult_float(result, result, y, maskPositiveExponent);
      // decrement exponent counters
      _cs149_vsub_int(exponentCounter, exponentCounter, one,
                      maskPositiveExponent);
    }

    // clamp values
    _cs149_vgt_float(maskLargeValues, result, maxValues, maskAll);
    _cs149_vmove_float(result, maxValues, maskLargeValues);
...
float arraySumVector(float *values, int N) {
  ...
  // // for (int i = 0; i < N; i += VECTOR_WIDTH) {
  // __cs149_vec_float result;
  // _cs149_vload_float(result, values + 0, maskAll);
